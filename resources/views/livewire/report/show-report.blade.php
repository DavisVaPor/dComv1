<div class="mt-4">
    <div class="flex justify-between items-center">
        <div>  
            <x-jet-button wire:click="regresar">
        ...
        <div class="flex justify-center mt-2">
            <div>
                @if ($report->estado == 'BORRADOR')
                    <x-jet-button wire:click="mostrarPre({{ $report->id }})" class="bg-green-500">
                ...
                @else
                    @if ($report->estado == 'PRESENTADO')
                        <x-jet-button wire:click="mostrarPen({{ $report->id }})" class="bg-yellow-700">
                            <p class="font-bold text-xs">Pendiente</p>
                        </x-jet-button>

                        @if (Auth::user()->rol->name == 'Director(a)')
                            <x-jet-button wire:click="mostrarRev({{ $report->id }})" class="bg-blue-700">
            ...
            <x-jet-button onclick="location.reload()" class="bg-blue-500 ml-2">
    ...
    {{-- Modal de Presentar --}}
    <x-jet-dialog-modal wire:model="modalPre">
        <x-slot name="title">
        ...
        <x-slot name="footer">
            <x-jet-secondary-button wire:click="$set('modalPre',false)" wire:loading.attr="disabled">
                {{ __('Cancel') }}
            </x-jet-secondary-button>

            <x-jet-button class="ml-2" wire:click="Presentar()" onclick="location.reload()"
    ...
    <x-jet-dialog-modal wire:model="modalPen">
        ...
        <x-slot name="footer">
            ...
            <x-jet-button class="ml-2" wire:click="Borrador()" onclick="location.reload()" wire:loading.attr="disabled">
                {{ __('Si') }}
            </x-jet-button>
        </x-slot>
    </x-jet-dialog-modal>
    ...
    {{-- Modal de Revisado --}}
    <x-jet-dialog-modal wire:model="modalRev">
        <x-slot name="title">
        ...
        <x-slot name="footer">
            <x-jet-secondary-button wire:click="$set('modalRev',false)" wire:loading.attr="disabled">
                {{ __('Cancel') }}
            </x-jet-secondary-button>

            <x-jet-button class="ml-2" wire:click="Revisar()" onclick="location.reload()" wire:loading.attr="disabled">
                {{ __('Si') }}
            </x-jet-button>
        </x-slot>
    </x-jet-dialog-modal>
